...
class ContractExtensionService {
   ...
   */
  async requestExtension(data: ExtensionRequest): Promise<ContractExtension> {
    const response = await axiosClient.post<ContractExtension>('/contract-extensions/request', data);
   ...
   */
  async getExtensionById(extensionId: number): Promise<ContractExtension> {
    const response = await axiosClient.get<ContractExtension>(`/contract-extensions/${extensionId}`);
    return response.data;
  }
   ...
   */
  async getExtensionsByContract(contractId: number): Promise<ContractExtension[]> {
    const response = await axiosClient.get<ContractExtension[]>(`/contract-extensions/contract/${contractId}`);
    return response.data;
  }
   ...
   */
  async getPendingExtensions(): Promise<ContractExtension[]> {
    const response = await axiosClient.get<ContractExtension[]>('/contract-extensions/pending');
    return response.data;
  }
   ...
   */
  async getPendingSignatureExtensions(): Promise<ContractExtension[]> {
    const response = await axiosClient.get<ContractExtension[]>('/contract-extensions/pending-signature');
    return response.data;
  }
   ...
   */
  async reviewExtension(extensionId: number, data: ExtensionReview): Promise<ContractExtension> {
    const response = await axiosClient.post<ContractExtension>(`/contract-extensions/${extensionId}/review`, data);
    return response.data;
  }
   ...
   * Approve extension
   */
  async approveExtension(extensionId: number, data: { newMonthlyPayment?: number; notes?: string } = {}): Promise<ContractExtension> {
    const response = await axiosClient.post<ContractExtension>(`/contract-extensions/${extensionId}/approve`, data);
    return response.data;
  }
   ...
   * Reject extension
   */
  async rejectExtension(extensionId: number, data: { reason: string }): Promise<ContractExtension> {
    const response = await axiosClient.post<ContractExtension>(`/contract-extensions/${extensionId}/reject`, data);
    return response.data;
  }
   ...
   */
  async cancelExtension(extensionId: number): Promise<ApiResponse<undefined>> {
    const response = await axiosClient.post<ApiResponse<undefined>>(`/contract-extensions/${extensionId}/cancel`);
    return response.data;
  }
   ...
   */
  async getMyExtensions(): Promise<ContractExtension[]> {
    const response = await axiosClient.get<ContractExtension[]>('/contract-extensions/my-extensions');
    return response.data;
  }
   ...
   */
  calculateNewEndDate(currentEndDate: string, durationMonths: number): Date {
   ...
   */
  calculateAdditionalCost(monthlyPayment: number, durationMonths: number): number {
   ...
   */
  validateExtensionRequest(data: ExtensionRequest): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
   ...
   */
  getStatusDisplay(status: string): ExtensionStatusDisplay {
    const statusMap: Record<string, ExtensionStatusDisplay> = {
      'PENDING': {
        text: 'Chờ duyệt',
        color: 'orange',
   ...
   */
  canCancelExtension(extension: ContractExtension): boolean {
   ...
   */
  canReviewExtension(extension: ContractExtension): boolean {
    return extension.status === 'PENDING';
  }
   ...
   */
  generateExtensionSummary(extension: ContractExtension, contract: Contract): ExtensionSummary {
   ...
   */
  getReasonTemplates(): ReasonTemplate[] {
    return [
   ...
   */
  checkExtensionEligibility(contract: Contract): { eligible: boolean; reason: string | null } {
    if (!contract) {
