...
export default function RenterInboundList() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await inventoryService.getInventoryRequests({ type: "INBOUND", pageSize: 100 });
  ...
  const exportCSV = () => {
    const rows = [
      ...
      ...filtered.map(r => [
        ...
        r.warehouseName || "",
        r.items?.[0]?.itemName || "",
        r.totalItems || r.items?.reduce((s, i) => s + i.quantity, 0) || 0,
  ...
  return (
    <div style={{ fontFamily: "Inter,sans-serif" }} className="w-full flex-1 flex flex-col min-w-0">
      ...
      {conf && (
        <Confirm
          msg={`Xóa yêu cầu nhập kho #${conf.id}? Hành động này không thể hoàn tác.`}
          label="Xóa"
          danger
