...
const INIT_DATA = [
  { id: "#IN-1024", warehouse: "Kho Quận 7 - TP.HCM", item: "Laptop Dell XPS 15", qty: 50, unit: "Chiếc", arrivalDate: "2024-11-20", status: "Đang chờ", created: "2024-11-10", notes: "Hàng dễ vỡ" },
  { id: "#IN-1023", warehouse: "Kho Sóng Thần - Bình Dương", item: "Màn hình Samsung 27\"", qty: 120, unit: "Chiếc", arrivalDate: "2024-11-18", status: "Đã duyệt", created: "2024-11-08", notes: "" },
  { id: "#IN-1022", warehouse: "Kho Cảng Hải Phòng", item: "Bàn phím cơ Keychron", qty: 300, unit: "Chiếc", arrivalDate: "2024-11-15", status: "Đang vận chuyển", created: "2024-11-05", notes: "Giao buổi sáng" },
  { id: "#IN-1021", warehouse: "Kho Hòa Lạc - Hà Nội", item: "Tai nghe Sony WH-1000XM5", qty: 80, unit: "Chiếc", arrivalDate: "2024-11-12", status: "Đã hủy", created: "2024-11-02", notes: "" },
  { id: "#IN-1020", warehouse: "Kho Quận 7 - TP.HCM", item: "Ghế công thái học ErgoPlus", qty: 25, unit: "Chiếc", arrivalDate: "2024-11-25", status: "Đang chờ", created: "2024-11-12", notes: "Lắp ráp tại kho" },
  { id: "#IN-1019", warehouse: "Kho Sóng Thần - Bình Dương", item: "Cáp HDMI 2.1", qty: 500, unit: "Chiếc", arrivalDate: "2024-11-28", status: "Từ chối", created: "2024-11-11", notes: "" },
];

const STATUS_CFG = {
  "Đang chờ":        { bg: "#fef3c7", color: "#92400e", dot: "#f59e0b" },
  "Đã duyệt":        { bg: "#d1fae5", color: "#065f46", dot: "#10b981" },
  "Đang vận chuyển": { bg: "#dbeafe", color: "#1e40af", dot: "#3b82f6" },
...
const EditModal = ({ row, onClose, onSave }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "1rem" }}>
      <div style={{ backgroundColor: "#fff", borderRadius: "16px", width: "100%", maxWidth: "520px", boxShadow: "0 20px 60px rgba(0,0,0,0.2)", overflow: "hidden" }}>
        <div style={{ padding: "18px 24px", borderBottom: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <h3 style={{ margin: 0, fontSize: "1rem", fontWeight: 800, color: "#111827" }}>Chỉnh sửa yêu cầu</h3>
        ...
        <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: "14px" }}>
          ...
          <div><label style={{ display: "block", fontSize: "0.75rem", fontWeight: 700, color: "#64748b", textTransform: "uppercase", marginBottom: "6px" }}>Ngày dự kiến hàng đến</label><input type="date" value={f.arrivalDate} onChange={set("arrivalDate")} style={inputSt} /></div>
          <div><label style={{ display: "block", fontSize: "0.75rem", fontWeight: 700, color: "#64748b", textTransform: "uppercase", marginBottom: "6px" }}>Ghi chú</label><textarea value={f.notes} onChange={set("notes")} rows={3} style={{ ...inputSt, resize: "none" }} placeholder="Hướng dẫn xử lý..." /></div>
        </div>
...
const STATUSES = ["Tất cả","Đang chờ","Đã duyệt","Đang vận chuyển","Đã hủy","Từ chối"];
...
export default function RenterInboundList() {
  ...
  const filtered = data
    .filter(r => {
      ...
      return (!q || [r.id,r.item,r.warehouse].some(v=>v.toLowerCase().includes(q))) && (sf==="Tất cả"||r.status===sf);
    })
    .sort((a,b)=>{
  ...
  const dup = row => { setData(d=>[{...row,id:`#IN-${Math.floor(1000+Math.random()*9000)}`,status:"Đang chờ",created:new Date().toISOString().slice(0,10)},...d]); };
  const exportCSV = () => {
    const rows=[["ID","Kho","Mặt hàng","SL","Đơn vị","Ngày đến","Trạng thái","Ngày tạo"],...filtered.map(r=>[r.id,r.warehouse,r.item,r.qty,r.unit,r.arrivalDate,r.status,r.created])];
    const a=document.createElement("a"); a.href="data:text/csv;charset=utf-8,"+encodeURIComponent(rows.map(r=>r.join(",")).join("\n")); a.download="nhap-kho.csv"; a.click();
  };
  ...
  return (
    <div style={{ fontFamily: "Inter,sans-serif" }} className="w-full flex-1 flex flex-col min-w-0">
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"22px", flexWrap:"wrap", gap:"12px" }}>
        <div>
          <h1 style={{ fontSize:"1.55rem", fontWeight:800, margin:0, color:"#111827" }}>Danh sách yêu cầu nhập kho</h1>
          <p style={{ margin:"6px 0 0", fontSize:"0.85rem", color:"#64748b" }}>Theo dõi và quản lý tất cả các yêu cầu nhập kho của bạn.</p>
        </div>
        <div style={{ display:"flex", gap:"8px", flexWrap:"wrap" }}>
          ...
          <Link to="/create-inbound" style={{ display:"flex", alignItems:"center", gap:"5px", padding:"9px 16px", borderRadius:"8px", backgroundColor:"#00b2d6", color:"#fff", fontWeight:700, fontSize:"0.875rem", textDecoration:"none" }}>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead>
              <tr style={{ backgroundColor:"#f8fafc" }}>
                ...
                <th style={th} onClick={()=>doSort("arrivalDate")}>Ngày đến <SortIco k="arrivalDate"/></th>
                <th style={th}>Trạng thái</th>
                <th style={th} onClick={()=>doSort("created")}>Ngày tạo <SortIco k="created"/></th>
            ...
            <tbody>
              {filtered.length===0 ? (
                ...
                <tr key={row.id} style={{ backgroundColor:sel.includes(row.id)?"#f0fdfa":"#fff" }}>
                  ...
                  <td style={{...td,maxWidth:"170px"}}><div style={{ overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontWeight:500 }}>{row.warehouse}</div></td>
                  ...
                  <td style={{...td,color:"#64748b"}}>{row.arrivalDate}</td>
                  <td style={td}><Badge s={row.status}/></td>
                  <td style={{...td,color:"#64748b"}}>{row.created}</td>
