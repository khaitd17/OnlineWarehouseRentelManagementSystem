...
function ConfirmToggleModal({ staff, isActive, onConfirm, onClose, busy }) {
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()}
      ...
      <div style={{
        background: C.surface, borderRadius: 14, padding: "24px 28px",
        width: "100%", maxWidth: 400, border: `1px solid ${C.border}`, boxShadow: C.shadowM
      }}>
        <div style={{ fontSize: 20, marginBottom: 10, textAlign: "center", fontWeight: "bold" }}>
          {isActive ? "Chú ý" : "Xác nhận"}
        </div>
    ...
    <div onClick={e => e.target === e.currentTarget && onClose()}
      ...
      <div style={{
        ...
        {error && <div style={{
          ...
          borderRadius: 8, padding: "9px 13px", fontSize: 12, marginBottom: 14
        }}>{error}</div>}

        {/* Role */}
        <label style={{
          fontSize: 11, fontWeight: 700, color: C.sub, textTransform: "uppercase",
          letterSpacing: ".6px", marginBottom: 8, display: "block"
        }}>Role trong kho</label>
        {isOperator ? (
          <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
            {["STAFF", "MANAGER"].map(r => (
              <button key={r} type="button"
                ...
                style={{
                  padding: "7px 16px", borderRadius: 20, fontWeight: 600, fontSize: 12, cursor: "pointer",
          ...
          <div style={{
            marginBottom: 18, padding: "8px 13px", background: C.card,
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 22 }}>
          <button onClick={onClose}
            style={{
              flex: 1, padding: "9px 0", background: "transparent", color: C.sub,
          ...
          <button onClick={handleSubmit} disabled={saving}
            style={{
              flex: 1, padding: "9px 0", background: C.accent, color: "#fff",
  ...
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()}
      ...
      <div style={{
        ...
        <div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 3 }}>Cấu hình ca làm việc</div>
        <div style={{ fontSize: 12, color: C.sub, marginBottom: 18 }}>{staff.fullName}</div>
        ...
        <select value={selectedShiftId} onChange={e => setSelectedShiftId(e.target.value)}
          style={{
            ...
            background: C.surface, color: C.text, fontSize: 13, marginBottom: 22
          }}>
...
function StaffCard({ staff, warehouseId, callerMembership, warehouseOptions, warehouseShifts, onRefresh }) {
  ...
  const canConfigShift = callerMembership &&
    (callerRoles.has("OWNER") || callerRoles.has("OPERATOR") || (callerRoles.has("MANAGER") && staff.roleCode === "STAFF")) &&
  ...
  return (
    <>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ marginTop: 6 }}>
            <span style={{ fontSize: 10, color: C.subL, textTransform: "uppercase", letterSpacing: ".5px", marginRight: 6 }}>Ca làm việc</span>
            {staff.warehouseShiftName ? (
              <span style={{ fontSize: 12, fontWeight: 600, color: C.accent }}>{staff.warehouseShiftName}</span>
            ) : (
              <span style={{ fontSize: 12, color: C.sub }}>Ca xoay (Không cố định)</span>
            )}
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 7, flexShrink: 0 }}>
          ...
          {canConfigShift && (
            <button onClick={() => setShowShiftConfig(true)}
              style={{
                padding: "6px 12px", borderRadius: 8, border: `1px solid ${C.accent}`,
                background: "#eff0ff", color: C.accent, cursor: "pointer", fontSize: 11, fontWeight: 700
              }}>
              Ca làm việc
            </button>
          )}
      ...
      {showShiftConfig && (
        <ShiftConfigModal staff={staff} warehouseShifts={warehouseShifts}
          onClose={() => setShowShiftConfig(false)}
          onSuccess={() => { setShowShiftConfig(false); onRefresh(); }} />
      )}
      {showConfirm && (
        <ConfirmToggleModal
          staff={staff}
...
export default function ListStaff() {
  ...
  useEffect(() => {
    if (!selectedWarehouse) { setCallerMembership(null); setWarehouseShifts([]); return; }
    Promise.all([
      staffService.getMyMembership(parseInt(selectedWarehouse)),
      staffService.getWarehouseOptions(parseInt(selectedWarehouse)),
      warehouseShiftService.getWarehouseShifts(parseInt(selectedWarehouse)),
    ]).then(([m, o, s]) => {
      setCallerMembership(m);
      setWarehouseOptions(o);
      setWarehouseShifts(s || []);
    }).catch(console.error);
  }, [selectedWarehouse]);
